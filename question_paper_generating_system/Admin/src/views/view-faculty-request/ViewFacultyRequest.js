...
const ViewFacultyRequest = () => {
  ...
  useEffect(async () => {
    ...
    await fetch('http://localhost:5000/api/user/allFaculties', {
      method: 'GET',
      headers: {
  ...
  const approveFaculty = async (id) => {
    // alert(id)
    if (window.confirm('Sure, you want to approve faculty request?') == true) {
      let data1 = JSON.parse(localStorage.getItem('admin-info'))
                  ...
                  {FacultyRequest.map((item, index) => (
                    <CTableRow v-for="item in tableItems" key={index}>
                      ...
                      <CTableDataCell className="text-center">
                        <CButton
                          onClick={() => approveFaculty(item._id)}
                          color="primary"
                          variant="outline"
                        >
                          Approve
